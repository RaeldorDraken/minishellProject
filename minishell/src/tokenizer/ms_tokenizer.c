...
// void	msh_create_new_cmd(char *cmd, int *c, char *i, char **s, int num)
// {
// 	cmd = msh_strjoinchr(cmd, i[*c]);
// 	cmd = msh_strjoinchr(cmd, i[*c]);
// 	*c++;
// 	s[num] = cmd;
// 	cmd = NULL;
...
char	**msh_do_split_input_in_cmds(t_vars *v, char **splitted, char *cmd)
{
	while (v->input[v->c->c1])
	{
			...
			&& !v->quotes->quote && !v->quotes->miniquote)
		{
			...
			if (cmd)
				cmd = NULL;
			if ((v->input[v->c->c1] == '<' && v->input[v->c->c1 + 1] == '<')
...
char	**msh_split_cmd_argvs(char *input, int argc)
{
	...
	while (input[++c])
	{
		if (is_startarg(input, c, &quotes))
			start = c;
		msh_update_quotes_status(&quotes, input[c]);
		if (is_endarg(input, c, &quotes))
		{
